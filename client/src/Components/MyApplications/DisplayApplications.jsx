/* eslint-disable react/prop-types */

import { Link } from "react-router-dom";

const DisplayApplications = ({ application }) => {
  const {
    _id: id,
    countryName,
    countryFlag,
    visaType,
    firstName,
    lastName,
    appliedDate,
  } = application;
  return (
    <div>
      <div className="card bg-[#eff2e6] max-w-80 mx-auto py-6 px-2 custom-card">
        <figure className="h-16 w-16 mt-8 ml-10">
          <img
            src={countryFlag}
            alt="flag"
            className="w-full h-full object-cover rounded-[50%]"
          />
        </figure>
        <div className="card-body pt-5">
          <h2 className="card-title text-2xl font-bold">{countryName}</h2>
          <p className="font-semibold text-lg">Type: {visaType}</p>
          <p className="font-semibold text-lg">Applied: {appliedDate}</p>
          <p className="font-semibold text-lg">Applicant: {firstName} {lastName}</p>
          <p></p>
          <div className="card-actions mt-5">
            <Link to={`/applicationDetails/${id}`} className="w-full">
              <button className="btn w-full hover:bg-[#898d4fbe] text-lg font-bold custom-effect2">
                <span className="z-10">See Details</span>
              </button>
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
};

export default DisplayApplications;
